package com.example.axadulin.galleryforyandex;
...
import com.example.axadulin.galleryforyandex.adapter.GridLayoutAdapter;
import com.example.axadulin.galleryforyandex.helper.Constants;
import com.example.axadulin.galleryforyandex.helper.GridSpaceItemDec;
import com.example.axadulin.galleryforyandex.helper.Helper;
import com.example.axadulin.galleryforyandex.helper.OnItemClickListener;
import com.example.axadulin.galleryforyandex.helper.SlideshowDialogFragment;
...
public class MainActivity extends AppCompatActivity implements OnItemClickListener {
    ...
    private ArrayList<FlickrPicture> mFlickrPictureList = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new GridLayoutAdapter(this, mFlickrPictureList, this);

        GridLayoutManager layoutManager = new GridLayoutManager(this, 2);
        mRecyclerView.setLayoutManager(layoutManager);
        mRecyclerView.addItemDecoration(new GridSpaceItemDec(2,
                Helper.getInstance().dpToPx(this, 8), true));
        mRecyclerView.setItemAnimator(new DefaultItemAnimator());
        mRecyclerView.setAdapter(mAdapter);

        //Log.e(TAG, "Checking network : " + Helper.getInstance().isNetworkAvailable(this));
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (mFlickrPictureList != null && !mFlickrPictureList.isEmpty()) {
    ...
    private void setupSearch() {
        ...
        mSearchView.setOnSearchViewListener(new MaterialSearchView.SearchViewListener() {
            ...
            public void onSearchViewClosed() {
                mAdapter.setImageList(mFlickrPictureList);
    ...
    private void processQuery(String query) {
        List<FlickrPicture> result = new ArrayList<>();

        if (mFlickrPictureList == null || mFlickrPictureList.isEmpty() || query.equals("")) {
            return;
        }
        // case insensitive search
        for (FlickrPicture flickrPicture : mFlickrPictureList) {
            String title = flickrPicture.getTitle();
            if (title.toLowerCase().contains(query.toLowerCase())) {
                result.add(flickrPicture);
            }
        }
    ...
    public void onItemClick(View v, int pos) {
        ...
        bundle.putSerializable("images", mFlickrPictureList);
    ...
    static  class JSONDownloaderTask extends AsyncTask<Void, Void, List<FlickrPicture>> {
        ...
        protected void onPreExecute() {
            ...
            if (errorMsgLayout.getVisibility() == View.VISIBLE &&
                    Helper.getInstance().isNetworkAvailable(mActivity)) {
                mActivity.mProgressBar.setVisibility(View.VISIBLE);
                errorMsgLayout.setVisibility(View.GONE);
            }
        }
        ...
        @Override
        protected List<FlickrPicture> doInBackground(Void... params) {
            String url = Constants.FLICKER_QUERY_URL
                    + Constants.FLICKR_QUERY_PER_PAGE
                    + Constants.FLICKR_QUERY_PAGES
                    + Constants.FLICKR_QUERY_NOJSONCALLBACK
                    + Constants.FLICKR_QUERY_FORMAT
                    + Constants.FLICKR_QUERY_KEY + Constants.FLICKER_API_KEY;
            List<FlickrPicture> postsArray = null;
            try {
        ...
        private List<FlickrPicture> parseJSON(String json){

            List<FlickrPicture> flickrPictureList = new ArrayList<>();
            ...
            try {
                ...
                for (int i = 0; i < JsonArray_photo.length(); i++){
                    ...
                    String FlickrPhotoURI = "http://farm" + flickrFarm + ".static.flickr.com/"
                                    + flickrServer + "/" + flickrId + "_" + flickrSecret + "_q.jpg";
                    String zFlickrPhotoURI="http://farm" + flickrFarm + ".static.flickr.com/"
                            + flickrServer + "/" + flickrId + "_" + flickrSecret + "_z.jpg";
                    flickrPicture = new FlickrPicture(FlickrPhotoURI, zFlickrPhotoURI,flickrTitle);
                    flickrPictureList.add(flickrPicture);
                }
            ...
            return flickrPictureList;
        }

        @Override
        protected void onPostExecute(List<FlickrPicture> flickrPictureList) {
            MainActivity mActivity = mContext.get();
            if (mActivity == null || mActivity.isFinishing() || isCancelled()) {
                return;
            }

            mActivity.mFlickrPictureList = (ArrayList<FlickrPicture>) flickrPictureList;


            if (mActivity.mFlickrPictureList == null) {
                LinearLayout errorMsgLayout = mActivity.findViewById(R.id.no_internet_layout);
                mActivity.mProgressBar.setVisibility(View.GONE);
                errorMsgLayout.setVisibility(View.VISIBLE);
                return;
            }
            if (!mActivity.mFlickrPictureList.isEmpty()) {
                mActivity.mProgressBar.setVisibility(View.GONE);
                mActivity.mAdapter.setImageList(mActivity.mFlickrPictureList);
